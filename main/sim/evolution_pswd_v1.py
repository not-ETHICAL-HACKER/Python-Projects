...
class Creature:
    def __init__(self,id):
        ...
        self.fitness = cc * 0.20 + cp * 0.70 + random_noise * 0.10
...
while pswd not in [creature.pswd for creature in pswd_arr]:
    ...
    print(f"Gen {c}: {max(pswd_arr,key = lambda x:x.fitness).pswd} with fitness: {max(pswd_arr,key = lambda x:x.fitness).fitness}",end="\r")
    best_fitness_arr.append(max(pswd_arr,key = lambda x:x.fitness).fitness)
